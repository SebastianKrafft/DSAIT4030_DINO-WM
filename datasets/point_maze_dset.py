...
class PointMazeDataset(TrajDataset):
    def __init__(
        ...
        states = torch.load(self.data_path / "states.pth", weights_only=False).float()
        self.states = states
        self.actions = torch.load(self.data_path / "actions.pth", weights_only=False).float()
        self.actions = self.actions / action_scale  # scaled back up in env
        self.seq_lengths = torch.load(self.data_path /'seq_lengths.pth', weights_only=False)
    ...
    def _load_episode_embeddings(self, idx):
        emb_path = self.embedding_dir / f"patched_ep{idx+1:03d}.pt"
